 ...
 */
class CancelAppointmentAcceptanceTest extends BaseAcceptanceTest {

    @Test
    void cancelAppointment() {
        // Given: an appointment for doctor with id 1L, on 2022-01-01, from 15:00 to 16:30
        var appointmentEntity = createAndSaveAppointment(1L, "2022-01-01", "15:00", "16:30");

        // When: calling the REST API to cancel the appointment
        var response = testRestTemplate.exchange("/api/v1/appointments/%d".formatted(appointmentEntity.getId()), HttpMethod.DELETE, null, Void.class);

        // Then: response status is OK
        assertEquals(HttpStatus.OK, response.getStatusCode());

        // And: appointment has been deleted
